...
def _main_locked(args):
    ...
    # ── Watchdog del pipeline (Iter4+Iter9): detecta stale signals ─────────────
    # El sistema "no opero del 3 al 18 de mayo sin alerta". Esto previene eso.
    # ITER9 hardening: NO alertar si el JSON no parsea (eso es bug de pipeline,
    # no stale real — antes mandaba "Edad 999h Ultimo analyst desconocido" lo
    # cual confundia a NAF). Solo alertar con datos validos. Cooldown 4h.
    try:
        ...
        if _sig_path.exists():
            try:
                ...
                _gen_str = _sig_data.get("generated_at", "") or ""
                if _gen_str:
                ...
                logger.warning("Watchdog: JSON parse error (%s) — skip alerta", _pe)
        else:
            logger.warning("Watchdog: signals/latest.json no existe — skip alerta")

        if not _parse_ok:
            # Sin datos validos, no podemos asegurar que el sistema este stale.
            # Log y seguir. La proxima corrida posiblemente tenga el JSON OK
            # despues del _pull_state.
            logger.warning("Watchdog: sin datos validos para evaluar staleness — skip")
        else:
            # Cooldown: 4h entre alertas del mismo problema (iter9, antes 2h).
            _last_alert_ts = 0.0
            ...
            if _age_h > 24 and _cooldown_ok:
                ...
                logger.warning("WATCHDOG: signals stale %.0fh (alerta en cooldown 4h)", _age_h)
            else:
