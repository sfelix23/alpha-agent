...
- ✅ **iter39 deployado**: **BUY limit buffer 0.15% → 2.0%**. Diagnóstico daily 05-27: 3 de 4 BUYs quedaron stuck — el buffer +0.15% no aguantó un movimiento intradiario de +1.47% (DDOG en 10 min). Las órdenes quedaron atrás del bid → no fillearon → deployment 58% vs 95% predicho. **iter39 = +2% buffer** (marketable limit con margen real): catch movimientos rápidos sin renunciar al control de precio. Slippage máximo $5 sobre BUY de $250 — trivial vs el costo del cash drag. SELL queda en -0.15%.
- ✅ **iter40+41 deployado**: (40) auto-retry de BUYs stuck — si una orden queda 'new' >15min en el monitor, cancel + market re-submit (belt-and-suspenders sobre iter39). (41) alerta WhatsApp/Telegram si posición individual ≤-6% (margen sobre backstop iter33 -8%) para que veas a tiempo antes del cierre forzado. Helpers puros + tests (48 total). Anti-spam por ticker.
- 📊 **Resultado del daily 05-27 (iter35 1er trading real)**: iter35 generó 4 BUYs (vs 1 ayer iter34 — cap+floor funcionó). MU FILLED (+$485 desplegados, equity +1.13% hoy = $1,769.88). DDOG/LLY/DE STUCK por el buffer tight → cash drag parcial $725 (58% deployment). Las pending expiran EOD 20:00 UTC. iter39 evita esto mañana.
- ⚠️ **JUEVES 05-28 — verificar el 1er daily con iter39**: que los 4-5 BUYs fillen en segundos, deployment 90-95%, cash idle <$100. `python run_dashboard.py --health`.
